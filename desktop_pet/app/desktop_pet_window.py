...
class ChatWorker(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            messages = self.prompt_builder.build_messages(self.user_message, recent_messages)
...
class DesktopPetWindow(QWidget):
    ...
    def _show_context_menu(self, global_pos: QPoint) -> None:
        ...
        menu = build_context_menu(
            ...
            on_clear_history=self._clear_chat_history,
    ...
    def _handle_user_message(self, message: str) -> None:
        ...
        if not self._api_chat_enabled():
            reply = self._generate_local_reply(message)
            self.chat_store.append_message("assistant", reply)
            self.sprite_player.set_action("idle")
            self._display_message(reply, 9000, "assistant")
            return
        ...
        if not self.deepseek_client.is_configured():
            ...
            self._display_message(reply, 14000, "assistant")
            return

        self._start_chat_worker(message)

    def _generate_local_reply(self, message: str) -> str:
        """在关闭 API 时，根据用户输入生成一条本地简短回复。"""
    ...
    def _on_chat_success(self, reply: str) -> None:
        ...
        self.sprite_player.set_action("idle")
        self._display_message(cleaned_reply, 15000, "assistant")
    ...
    def _trigger_auto_move(self) -> None:
        ...
        target_x = max(available.left(), min(current.x() + delta, available.right() - self.width()))
